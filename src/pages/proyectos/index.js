...
const Proyectos = () => {
  const data = Array(49).fill(0).map((_, index) => `/projects/projects-${index + 1}.jpg`)
  ...
  return (
    <div>
      <Head>
        <title>Techoland | Proyectos</title>
      </Head>
      ...
      <Modal show={modalShow} onHide={() => setModalShow(false)} title={`Proyecto ${imageSelected.index + 1} / ${data.length}`}>
        <button className="absolute top-1/2 right-0 opacity-60 md:-right-14 md:opacity-100" onClick={() => { setImageSelected(handleImage()) }}>
          <svg className="w-8 h-8 md:w-14 md:h-14 fill-gray-500 md:fill-white" viewBox="0 0 26 43" fill="none" xmlns="http://www.w3.org/2000/svg">
